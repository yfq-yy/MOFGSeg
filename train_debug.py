...
def main():
    ...
    # laplace = torch.tensor([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]]).cuda()
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x, edge_x), img_u_s) in enumerate(loader):
            ...
            # unique_values = np.unique(mask_x.cpu().numpy()) #yfq
            # print(unique_values)
            # unique_value2 = np.unique(edge_x.cpu().numpy()) #yfq
            # print(unique_value2)
            mask_x[mask_x==255] = -1
            ...
            # unsup_loss = compute_supervised_loss(pred_u, pseudo_labels)
        # apply regional contrastive loss
            ...
            loss = sup_loss + unsup_loss + reco_loss + sup_edge_loss
