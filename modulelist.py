...
# Define ReCo loss
# --------------------------------------------------------------------------------
def compute_reco_loss(rep, label, mask, prob, strong_threshold=1.0, temp=0.5, num_queries=64, num_negatives=128):
    ...
    if len(seg_num_list) <= 1:  # in some rare cases, a small mini-batch might only contain 1 or no semantic class
        ...
        for i in range(valid_seg):
            ...
            weights = torch.tensor([1.0] + [2.0] * 512).to(device)
            seg_logits = seg_logits * weights
...
#             class_mask = np.uint8(map[i] == class_id) * 255  # 创建当前类别的二值图像
#             class_edges = cv2.Canny(class_mask, threshold1=100, threshold2=200)
#             edges[i] = np.maximum(edges[i], class_edges)

#         # 保存边缘图到输出目录
#     output_path = "/data/fyao309/AllSpark/exp/oucuavseg/allspark/1_2_test"
#     cv2.imwrite(output_path, edges)

    # edge = np.zeros((b, h, w))
    # for i in range(b):
    #     edge[i] = filters.sobel(map[i])
    # edge = torch.from_numpy(edge).cuda()
    # return edge
  
    # edge = np.zeros((b, 1, h, w))
    # for i in range(b):
    #     edge[i] = F.conv2d(map[i], laplace, padding=1)
    #     edge[i] = F.relu(torch.tanh(edge[i]))
    #return edge


# def dice_edge_loss(predict, target):
#     smooth = 1
#     p = 2
#     valid_mask = torch.ones_like(target)
#     predict = predict.contiguous().view(predict.shape[0], -1)
