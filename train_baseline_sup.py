...
parser = argparse.ArgumentParser(description='finegrained')
...
#yfq    the output of model is a tuple
        ...
        for img, mask, id in loader:
            ...
            if mode == 'sliding_window':
                ...
                while row < h:
                    col = 0
                    while col < w:
                        pred,_,_ = model(img[:, :, row: min(h, row + grid), col: min(w, col + grid)])
...
def evaluatemask(model, loader, mode, cfg, datasets):
    model.eval()
    assert mode in ['original', 'center_crop', 'sliding_window']
    intersection_meter = AverageMeter()
    union_meter = AverageMeter()
    cmap = color_map(datasets)
    with torch.no_grad():
        for img, mask, id in loader:
            img = img.cuda()
            ...
            for label in np.unique(preds):
                masks = preds == label
                color_rgb = cmap[label]
                output_image[masks] = color_rgb
            output_image = Image.fromarray(output_image)
            mask_name = "mask" +id[0].split(' ')[1].split('/')[1]
            #yfq
            output_image_path = './exp/oucuavseg/finegrained/1_2/best/'+mask_name
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, (img, mask, edge) in enumerate(trainloader):

            img, mask, edge = img.cuda(), mask.cuda(), edge.cuda()
            pred, _, _ = model(img)
